# Contributing to umbrucore-lib

We're working hard to make *umbrucore-lib* the most powerful JavaScript library for working with Umbru. Our goal is to have *umbrucore-lib* be a library that can be used by anyone interested in Umbru, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.umbru.io/forum (Support Forum)
...
These are some global design goals in umbrucore-lib that any change must adhere.
...
When possible, umbrucore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of umbrucore-lib methods. For example, calling `JSON.stringify` on an umbrucore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'umbrucore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the umbrucore object.

DO:
```javascript
var umbrucore = require('../');
var PublicKey = umbrucore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add umbru git@github.com:umbru/umbrucore-lib.git
git pull --rebase umbru master
```
...
```
Finally go to [github.com/umbru/umbrucore-lib](https://github.com/umbru/umbrucore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of umbrucore-lib.

If you have any questions feel free to post them to
[github.com/umbru/umbrucore-lib/issues](https://github.com/umbru/umbrucore-lib/issues).
